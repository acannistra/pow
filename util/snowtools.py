...
SNOWDATA_URL="https://api.snowobs.com/wx/v1/station/data/timeseries"
...
def get_snow_df(staid, hours, snow_field = 'snow_depth'):
    ...
    req_params = {
        ...
        'start_date': (datetime.utcnow() - timedelta(hours=hours)).strftime("%Y%m%d%H%M")
    }

    raw = requests.get(SNOWDATA_URL, params=req_params)

    obs = raw.json()['STATION'][0]['observations']
    data = pd.DataFrame(
        data=obs
    )
